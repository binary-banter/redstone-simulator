...
pub fn write_dot(cblocks: &CBlockGraph, pos: &HashMap<NodeIndex, (isize, isize, isize)>, path: &Path) {
    ...
    for (node, cblock) in cblocks.node_references() {
        ...
        if let Some(pos) = pos.get(&node) {
            writeln!(w, "{}[label=\"{label}\",color={color}, world_pos=\"{:?}\"];", node.index(), pos).unwrap();
        } else {
    ...
    for edge in cblocks.edge_references() {
        let color = if edge.weight().is_side() {"#A9A9A9"} else {"#000000"};
        writeln!(w, "{} -> {}[label=\"{}\",color={color}];", edge.source().index(), edge.target().index(), edge.weight().strength_loss()).unwrap();
    }
