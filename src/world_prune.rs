...
impl World {
        ...
        loop {
            ...
            self.blocks.retain_nodes(|blocks, y| {
                ...
                match &blocks[y] {
                    ...
                    Block::Telescope(_) => blocks.neighbors_directed(y, Outgoing).count() > 0,
    ...
    fn telescopify(&mut self){
        self.blocks
            .node_indices()
            .collect_vec()
            .into_iter()
            .for_each(|idx| {
                // Check if node is a repeater with 1t delay or a torch.
                let t1 = match &self.blocks[idx] {
                    Block::Repeater(v) if v.delay() == 1 => Telescope::from_repeater(v),
                    Block::Torch(v) => Telescope::from_torch(v),
                    Block::Telescope(v) => v.clone(),
                    _ => return,
                };

                if self.blocks.neighbors_directed(idx, Incoming).count() > 1{
                    return
                }
                ...
                for n_idx in self.blocks.neighbors_directed(idx, Outgoing).collect_vec().into_iter() {
                    let t2 = match &self.blocks[n_idx] {
                        Block::Repeater(v) if v.delay() == 1 => Telescope::from_repeater(v),
                        Block::Torch(v) => Telescope::from_torch(v),
                        Block::Telescope(v) => v.clone(),
                        _ => continue,
                    ...
                    if self
                        .blocks
                        .edges_directed(n_idx, Outgoing)
                        .any(|edge| edge.weight().is_side())
                    {
                        continue;
                    };

                    let Ok(edge) = self.blocks.edges_directed(n_idx, Incoming).exactly_one().map(|e| e.id()) else {
                        continue;
                    };

                    if self.blocks[edge].is_side() {
                        continue;
                    }

                    *self.blocks.node_weight_mut(n_idx).unwrap() = Block::Telescope(Telescope::combine(&t1, &t2));

                    self.blocks.remove_edge(edge);
                    for edge in self.blocks.edges_directed(idx, Incoming).map(|e| e.id()).collect_vec().into_iter() {
                        let src = self.blocks.edge_endpoints(edge).unwrap().0;
                        let weight = self.blocks[edge];
                        self.blocks.add_edge(src, n_idx, weight);
                    }
                }
            });
    }
